	...
	public void setValues(BaseModel baseModel) {
		...
		try {
			if(baseModel != null && baseModel instanceof Message) {
				...
				if(!TextUtils.isEmpty(message.getPost_member_id())) {
//////////////////////////////////////////////////////////////////////////					
					//내가 보낸 메세지
...
//////////////////////////////////////////////////////////////////////////
					//다른 유저가 보낸 메세지.
						...
						if(!TextUtils.isEmpty(message.getPost_media_src())
								&& (ivProfile.getTag() == null || !ivProfile.getTag().toString().equals(message.getPost_media_src()))) {
							String key = ApplicationManager.getDownloadKeyFromTopFragment();
							ImageDownloadUtils.downloadImage(message.getPost_media_src(), key, ivProfile, 160);
						} else {
							ivProfile.setVisibility(View.VISIBLE);
						}
					}
...
//////////////////////////////////////////////////////////////////////////					
				...
				if(!TextUtils.isEmpty(message.getReg_dt())) {
				...
				if(message.getContent_type() == TYPE_TEXT) {
					...
					String key = ApplicationManager.getDownloadKeyFromTopFragment();
					ImageDownloadUtils.downloadImageImmediately(message.getContent(), key, ivImage, 400, true);
				}
			}
		} catch(Exception e) {
			e.printStackTrace();
