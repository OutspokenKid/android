...
public class ButtonForGrid extends FrameLayout {
	...
	public void setButtonByMenu(GridMenu gridMenu) {
		...
		if(!TextUtils.isEmpty(gridMenu.getImageUrl())) {
		...
		if(!TextUtils.isEmpty(gridMenu.getUri())) {
			try {
				//Set onClickListener.
				ToastUtils.showToast("menuClicked : " + gridMenu.getUri());
			} catch(Exception e) {
		...
		if(!TextUtils.isEmpty(gridMenu.getIconUrl())) {
			try {
				//Download icon.
				centerText = false;
				viewForImage = new View(getContext());
		...
		if(!TextUtils.isEmpty(gridMenu.getText())) {
			try {
