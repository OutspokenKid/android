...
public abstract class BCPAuctionableListFragment extends BCPFragment {
	...
	//이전 상태 값 저장.
		int lastFirstVisibleItemPosition = listView.getFirstVisiblePosition();
		int previousItemId = 0;
		int lastItemId = 0;
		boolean removed = false;
		
		if(models.get(lastFirstVisibleItemPosition) instanceof Car) {
			lastItemId = ((Car)models.get(listView.getFirstVisiblePosition())).getId();
		}
		
		if(lastFirstVisibleItemPosition != 0) {
			
			if(models.get(lastFirstVisibleItemPosition - 1) instanceof Car) {
				previousItemId = ((Car)models.get(lastFirstVisibleItemPosition - 1)).getId();
			}
		}
		
		int lastTop = listView.getChildAt(0).getTop();
	...
	//스크롤 맞춰주기.
		...
		if(lastFirstVisibleItemPosition != listView.getFirstVisiblePosition()) {
			...
			if(removed) {
				for(int i=startIndex; i<models.size(); i++) {
					...
					if(((Car)models.get(i)).getId() == previousItemId) {
						targetPosition = i;
					}
				}
			} else {
				...
				for(int i=startIndex; i<models.size(); i++) {
					...
					if(((Car)models.get(i)).getId() == lastItemId) {
						targetPosition = i;
					}
				}
			}
			...
			if(AppInfoUtils.checkMinVersionLimit(android.os.Build.VERSION_CODES.HONEYCOMB)) {
				listView.smoothScrollToPositionFromTop(targetPosition, lastTop, 0);
			} else {
