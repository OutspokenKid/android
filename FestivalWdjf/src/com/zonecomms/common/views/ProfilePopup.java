...
public class ProfilePopup extends FrameLayout {
		...
		ivImage.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(myStoryInfo != null && !StringUtils.isEmpty(myStoryInfo.getMystory_member_profile())) {
					...
					ApplicationManager.getInstance().getActivity().showImageViewerActivity(title, imageUrls, null, 0);
				}
			}
		});
	...
	public void show(final String userId) {
		...
		ApplicationManager.getInstance().getActivity().checkLoginAndExecute(new OnAfterLoginListener() {
