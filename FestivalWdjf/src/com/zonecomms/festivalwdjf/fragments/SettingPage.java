...
import com.zonecomms.festivalwdjf.BaseFragmentActivity.OnPositiveClickedListener;
...
/////////////////////// Custom methods.
	...
	public void addFrameForEditPush() {
		...
		all = new View(mContext);
		all.setBackgroundResource(R.drawable.img_checkbox_off);
		ResizeUtils.viewResize(50, 50, all, 2, Gravity.RIGHT|Gravity.TOP, new int[]{0, 25, 50, 0});
		frameForEditPush.addView(all);
		
		message = new View(mContext);
		ResizeUtils.viewResize(50, 50, message, 2, Gravity.RIGHT|Gravity.TOP, new int[]{0, 125, 50, 0});
		message.setBackgroundResource(R.drawable.img_checkbox_off);
		frameForEditPush.addView(message);
		
		reply = new View(mContext);
		ResizeUtils.viewResize(50, 50, reply, 2, Gravity.RIGHT|Gravity.TOP, new int[]{0, 225, 50, 0});
		reply.setBackgroundResource(R.drawable.img_checkbox_off);
		frameForEditPush.addView(reply);
		
		sound = new View(mContext);
		ResizeUtils.viewResize(50, 50, sound, 2, Gravity.RIGHT|Gravity.TOP, new int[]{0, 325, 50, 0});
		sound.setBackgroundResource(R.drawable.img_checkbox_off);
		frameForEditPush.addView(sound);
		
		vibration = new View(mContext);
		ResizeUtils.viewResize(50, 50, vibration, 2, Gravity.RIGHT|Gravity.TOP, new int[]{0, 425, 50, 0});
		vibration.setBackgroundResource(R.drawable.img_checkbox_off);
		frameForEditPush.addView(vibration);
	...
	public void showFrameForEditPush() {
		...
		AsyncStringDownloader.download(url, downloadKey, new OnCompletedListener() {
			...
			@Override
			public void onErrorRaised(String url, Exception e) {
				LogUtils.log("showEditPush.onError\nurl : " + url);
			}
			
			@Override
			public void onCompleted(String url, String result) {

				LogUtils.log("showEditPush.onCompleted\nurl : " + url + "\nresult : " + result);
	...
	public void editPushSetting() {
		...
		try {
			SharedPrefsUtils.addDataToPrefs(ZoneConstants.PREFS_PUSH, "sound", needSound);
			SharedPrefsUtils.addDataToPrefs(ZoneConstants.PREFS_PUSH, "vibration", needVibration);
		} catch(Exception e) {
			LogUtils.trace(e);
		}
				...
				public void onCompleted(String url, String result) {
					...
					try {
						if((new JSONObject(result)).getInt("errorCode") == 1) {
							...
							mActivity.finishFragment(SettingPage.this);
	...
	public void setChecked(int index, boolean checked) {
		...
		case 4:
			target = vibration;
			needVibration = checked;
			break;
		}
		
		if(checked) {
			target.setBackgroundResource(R.drawable.img_checkbox_on);
		} else{
			target.setBackgroundResource(R.drawable.img_checkbox_off);
		}
	}

	@Override
	protected String generateDownloadKey() {
		return "SETTINGPAGE" + madeCount;
	}
