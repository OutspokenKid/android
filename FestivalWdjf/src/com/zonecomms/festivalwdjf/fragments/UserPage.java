	...
	protected void createPage() {
		...
		gridView.setOnScrollListener(new OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				LogUtils.log("###UserPage.GridView.onScrollStateChanged.  scrollState : " + scrollState
						+ ",  FVP : " + gridView.getRefreshableView().getFirstVisiblePosition());
				
				if(scrollState == 0) {
					if(gridView.getRefreshableView().getFirstVisiblePosition() == 0) {
						LogUtils.log("###UserPage.onScrollStateChanged.  show1");
						showInformation();
					} else {
						LogUtils.log("###UserPage.onScrollStateChanged.  hide1");
						hideInformation();
					}
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				if(firstVisibleItem == 0 && gridView.getRefreshableView().getChildCount() != 0 
					...
					LogUtils.log("###UserPage.onScrollStateChanged.  show2");
					showInformation();
				} else if(firstVisibleItem == numOfColumn) {
					LogUtils.log("###UserPage.onScrollStateChanged.  hide2");
					hideInformation();
				}
			}
		});
...
//		rp.addRule(RelativeLayout.BELOW, madeCount + 3);
	...
	public void showInformation() {
		...
		LogUtils.log("###UserPage.showInformation.  iA : " + isAnimating + ", iIH : " + isInfoHidden);
