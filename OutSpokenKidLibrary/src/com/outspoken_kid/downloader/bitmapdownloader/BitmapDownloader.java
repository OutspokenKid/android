...
public class BitmapDownloader extends BaseAsyncDownloader {
	...
	public Bitmap getBitmapFromCache(String key) {
		...
		if(imageCache != null && !StringUtils.isEmpty(key)) {
	...
	@TargetApi(Build.VERSION_CODES.HONEYCOMB) protected void executeTask(String url, String key, OnCompletedListener listener, ArrayList<ImageFilter> arImageFilter,
			ImageView targetView, boolean useMemoryCache) {
	...
	public static void downloadImage(String _url, String key, ImageView _imageView, boolean isImmediately, boolean useMemoryCache) {
		...
		OnCompletedListener ocl = new OnCompletedListener() {
		...
		if(isImmediately) {
			downloadImmediately(_url, key, ocl, null, _imageView, useMemoryCache);
		} else {
			download(_url, key, ocl, null, _imageView, true);
		}
	}
