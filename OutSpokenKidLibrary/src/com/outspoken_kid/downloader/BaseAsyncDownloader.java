...
@TargetApi(Build.VERSION_CODES.HONEYCOMB) public abstract class BaseAsyncDownloader {
	...
	@TargetApi(Build.VERSION_CODES.HONEYCOMB) protected void checkIsDownloading() {
	...
	protected void downloadComplete(AsyncTask<Void, Void, Void> task) {
		...
		if(queue != null && queue.size() > 0 && queue.get(0) == task) {
			...
			if(queue.size() > 0) {
				...
				if (Build.VERSION.SDK_INT>=Build.VERSION_CODES.HONEYCOMB) {
					(queue.get(0)).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
				} else {
					(queue.get(0)).execute();
				}
			}
