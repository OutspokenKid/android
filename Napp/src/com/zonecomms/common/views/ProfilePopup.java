...
public class ProfilePopup extends FrameLayout {
	...
	private void init() {
		...
		int progressSize = ResizeUtils.getSpecificLength(80);
		int progressMargin = (width - ResizeUtils.getSpecificLength(80))/2;
		progress =  new ProgressBar(getContext());
		rp = new RelativeLayout.LayoutParams(progressSize, progressSize);
		rp.addRule(RelativeLayout.BELOW, madeCount);
		rp.addRule(RelativeLayout.ALIGN_PARENT_LEFT);
		rp.leftMargin = progressMargin;
		rp.topMargin = progressMargin;
		...
		ivImage.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(myStoryInfo != null && !StringUtils.isEmpty(myStoryInfo.getMystory_member_profile())) {
					...
					((MainActivity)ApplicationManager.getInstance().getActivity()).showImageViewerActivity(title, imageUrls, null, 0);
				}
			}
		});
		...
		try {
			...
			AsyncStringDownloader.OnCompletedListener ocl = new OnCompletedListener() {
				...
				public void onErrorRaised(String url, Exception e) {
					...
					LogUtils.log("ProfilePopup.onErrorRaised.\nurl : " + url);
				}
	...
	public void addFriend() {
		...
		try {
			...
			AsyncStringDownloader.OnCompletedListener ocl = new OnCompletedListener() {
				...
				@Override
				public void onErrorRaised(String url, Exception e) {
					LogUtils.log("ProfilePopup.addFriend.onError.\nurl : " + url);
				}
				
				@Override
				public void onCompleted(String url, String result) {
					LogUtils.log("ProfilePopup.addFriend.onCompleted.\nurl : " + url + "\nresult : " + result);
				}
			};
