...
public class GetheringPage extends BaseListFragment {
	...
	protected void setSizes() {
		...
		tvGetheringName.setMaxWidth(ResizeUtils.getScreenWidth()*2/3 - s*2);
		tvGetheringName.setPadding(s, s, s, s);
		
		rp = (RelativeLayout.LayoutParams) tvNickname.getLayoutParams();
		rp.width = LayoutParams.WRAP_CONTENT;
		rp.height = l1/2;
		tvNickname.setMaxWidth(ResizeUtils.getScreenWidth()/3 - s*2);
		tvNickname.setPadding(s, s, s, s);
						...
						if(!StringUtils.isEmpty(gethering.getReg_image())) {
							...
							BitmapDownloader.OnCompletedListener ocl = new BitmapDownloader.OnCompletedListener() {
								...
								public void onCompleted(String url, Bitmap bitmap, ImageView view) {
									...
									//Check tag.
									if(view.getTag() == null || !url.equals(view.getTag().toString())) {
										return;
									}
							...
							ivOwnerProfile.setTag(gethering.getReg_image());
							BitmapDownloader.download(gethering.getReg_image(), getDownloadKey(), 
									ocl, null, ivOwnerProfile, true);
						} else {
							progressForOwner.setVisibility(View.INVISIBLE);
						}
