...
public class IntroActivity extends Activity {
	...
	public void setSizes() {
		...
		ResizeUtils.viewResize(330, 32, company, 2, Gravity.CENTER_HORIZONTAL|Gravity.BOTTOM, new int[]{0, 0, 0, 30});
	}
...
////////////////////// Custom methods.
		...
		BitmapDownloader.OnCompletedListener ocl = new BitmapDownloader.OnCompletedListener() {
			...
			public void onCompleted(String url, Bitmap bitmap, ImageView view) {
				...
				if (Build.VERSION.SDK_INT>=Build.VERSION_CODES.HONEYCOMB) {
					bfc.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
				} else {
					bfc.execute();
				}
	...
	@TargetApi(Build.VERSION_CODES.HONEYCOMB) public void loadOldSponserBitmap() {
		...
		if (Build.VERSION.SDK_INT>=Build.VERSION_CODES.HONEYCOMB) {
			blfc.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
		} else {
			blfc.execute();
		}
	}
